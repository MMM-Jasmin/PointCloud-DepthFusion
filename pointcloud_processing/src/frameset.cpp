 ...
 */
void Frameset::allocateDepthFrame(const Intrinsics& intrinsics) { depth_frame.allocate(intrinsics, stream); }
 ...
 */
void Frameset::allocateColorFrame(const Intrinsics& intrinsics) { color_frame.allocate(intrinsics, stream); }

/**
 * @brief Copy depth frame from host to gpu.
 * @param depth_frame Depth frame host source
 */
void Frameset::setDepthFrame(const uint16_t* depth_frame) { this->depth_frame.copyToDevice(depth_frame); }
 ...
 */
void Frameset::setColorFrame(const uint8_t* color_frame) { this->color_frame.copyToDevice(color_frame); }

/**
 * @brief Get depth frame gpu device pointer.
 * @return Depth frame device pointer
 */
uint16_t* Frameset::getDepth() { return depth_frame.get(); }

/**
 * @brief Get unaligned depth frame gpu device pointer.
 * @return Unaligned depth frame device pointer
 */
uint16_t* Frameset::getDepthUnaligned() { return depth_frame.getUnaligned(); }

/**
 * @brief Get color frame gpu device pointer.
 * @return Color frame device pointer
 */
uint8_t* Frameset::getColor() { return color_frame.get(); }

/**
 * @brief Get frame mask gpu device pointer.
 * @return Frame mask device pointer
 */
uint8_t* Frameset::getMask() { return depth_frame.getMask(); }
 ...
 */
void Frameset::copyColorToHost(uint8_t* host_color_frame) { color_frame.copyToHost(host_color_frame); }

/**
 * @brief Copy depth frame from gpu to host.
 * @param host_depth_frame Depth frame host destination
 */
void Frameset::copyDepthToHost(uint16_t* host_depth_frame) { depth_frame.copyToHost(host_depth_frame); }
